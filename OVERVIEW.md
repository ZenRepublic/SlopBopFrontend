...
**Surviving the gateway — `src/config/arweave.ts`.** The gateway is not reliable. `turbo-gateway.com` 302s to a per-content sandbox subdomain, and that hop blips: measured at roughly 3 bad fetches in 12 of a hot URL, either a 504 or a stall the gateway itself only abandons after ~50s. A retry almost always lands, and lands in about a second, so **every path that pulls bytes from Arweave retries** and they share one policy — attempt count, delay, and a silence watchdog that measures *no bytes arriving* rather than elapsed time, so a slow connection is never cut off. The mechanics can't be shared (an `<img>` and an `HTMLAudioElement` load through their own `src`, not `fetch`), only the numbers: `Img` retries by remounting, `MusicPlayerContext` by re-assigning `src`, and genuine downloads go through `fetchArweave`.

Two things follow from this that look optional and aren't. `Img` sends `crossorigin="anonymous"` for this host, because an opaque response reports status `0` whatever really came back — with `statuses: [0, 200]` the service worker cached gateway 504s **as the image**, for 60 days, turning a blip permanent. And the audio element re-assigns `src` to retry rather than calling `play()` again: after a failed load it sits in `NETWORK_NO_SOURCE` where `play()` can only reject, which is why a failed song used to stay broken until a page reload.

**If media storage ever changes**, the only coupling is the `IMAGE_HOST` regex at the top of `vite.config.ts` and `GATEWAY_HOST` in `config/arweave.ts` — point both at the new gateway and nothing else changes. The `Img` component and every call site stay identical, because the service worker intercepts at the network layer, below React.
