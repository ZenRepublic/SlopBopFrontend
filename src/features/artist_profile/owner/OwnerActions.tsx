...
interface Props {
   ...
   */
  isOwner: boolean;
  /** The artist these actions act on — whatever profile they're overlaid on. */
  artistId: string;
}
 ...
 *
 * Create opens the create menu; settings is still inert. Create wears lime
 * because it's the one thing on the page only you can do; settings stays in the
 * back button's black, since it's chrome rather than the point.
 *
 ...
 */
export default function OwnerActions({ isOwner, artistId }: Props) {
  ...
  return (
    <div className="absolute top-lg right-lg z-10 flex items-center gap-sm">
      ...
      {/* Portals to the body, so it doesn't inherit the hero's stacking or the
